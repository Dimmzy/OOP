...
//more tests
        ...
        try {
            ...
            System.out.println("#114:Error in simplify:" + ex);
        }
        ex = new Div(10, 0);
        try {
        ...
        ex = new Sin(new Cos(new Sin(new Sin("x"))));
        if (!ex.differentiate("x").toString().equals("(cos(cos(sin(sin(x)))) * (-(sin(sin(sin(x))) * (cos(sin(x)) * (cos(x) * 1.0)))))")) {
            System.out.println("#117 Error in differ of:" + ex.differentiate("x'"));
        }
        ex = new Div("x", "x");
        try {
